...
struct BatchedMatMulAttributes {
  ...
  int constant_index = -1; // 0 if input0 is const, 1 if input1 is const, -1 if runtime
  BHWC constant_shape;
};
// pkshin end
...
struct QuantizeAndDequantizeAttributes {
  ...
  float max = 0;
  float scale = 0;
  bool is_fp16 = false; // [CRITICAL FIX] Pass precision context to kernels without modifying scale here.
};

struct GatherAttributes {
  Axis axis = Axis::UNKNOWN;
  // [ROOT CAUSE FIX] Added batch_dims to support GatherV2 and GatherV3
  int batch_dims = 0;
