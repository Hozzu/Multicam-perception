...
#include "tflite/delegates/gpu/gl/runtime/shared_buffer.h"
#include "tflite/delegates/gpu/gl/runtime_options.h"
#include "tflite/delegates/gpu/gl/stats.h"
...
class Runtime {
 public:
  ...
  // [Fix] Add an overload to directly accept a pre-compiled GlProgram from binary.
  // This bypasses the shader compilation phase entirely.
  absl::Status AddProgram(GlProgram program,
                          const std::vector<Variable>& parameters,
                          const std::vector<Object>& objects,
                          const uint3& num_workgroups);
